...
const confirmDialog = (position) => {
  confirm.require({
    ...
    position: position,
    rejectProps: { label: 'Отмена', outlined: true },
...
const saveConfirmPopup = (event) => {
  confirm.require({
    ...
    header: 'Подтверждение',
    rejectProps: { label: 'Отменить', outlined: true },
    acceptProps: { label: 'Сохранить' },
    accept: () => {
      toast.add({
        severity: 'info',
        summary: 'Подтверждено',
...
const settings = [
  {
    label: 'Опции',
    items: [
      { id: 0, label: 'Подробнее' },
      { id: 1, label: 'Уведомить' },
    ],
  },
]
...
const tieredSettings = ref([
  ...
  },
  {
    ...
    items: [{ label: 'Копировать' }, { label: 'Удалить' }],
  },
  { label: 'Поиск' },
  { separator: true },
  { label: 'Отправить', items: [{ label: 'Telegram' }, { label: 'WhatsApp' }] },
])
...
<template>
  <UiShowcaseTable
    ...
    <template #popover-default>
      ...
      <Popover ref="popover" class="app-popover">
        <div
          ...
          :key="element.name"
          class="cursor-pointer p-2 hover:bg-[var(--surface-100)]"
    ...
    <template #tooltip-default>
      <div class="flex max-w-[14rem] flex-wrap gap-4">
        <InputText
          v-tooltip.top="{ value: 'Дополнительная информация', showDelay: 500 }"
          type="text"
